...
def generate_packet(filter_file, ifmap1_file, ifmap2_file, sv_filename):
    ...
    for i in range(3):
        ...
        packets.append(v + [opcodes[i], '0', '10', '11'])
    # Ifmap windows (single SV concat)
    packets.append([int(x) for x in ifmap1_vals[:9]] + ['0000', '0', '10', '11'])
    packets.append([int(x) for x in ifmap2_vals[:9]] + ['0001', '0', '10', '11'])
    ...
    with open(sv_filename, 'w') as f:
        for idx, comps in enumerate(packets):
            if idx < 3:
                f.write(
                    f"send_values[{idx}] = {{8'd{comps[0]},8'd{comps[1]},8'd{comps[2]},"
                    f"4'b{comps[3]},1'b{comps[4]},2'b{comps[5]},2'b{comps[6]}}} // filter row {idx+1}\n"
                )
            elif idx == 3:
                bits = ",".join([f"1'b{b}" for b in comps[:9]])
                f.write(
                    f"send_values[{idx}] = {{{bits},4'b{comps[9]},1'b{comps[10]},2'b{comps[11]},2'b{comps[12]}}} // ifmap t1\n"
                )
            else:
                bits = ",".join([f"1'b{b}" for b in comps[:9]])
                f.write(
                    f"send_values[{idx}] = {{{bits},4'b{comps[9]},1'b{comps[10]},2'b{comps[11]},2'b{comps[12]}}} // ifmap t2\n"
                )

def write_binary_packets(filter_file, ifmap1_file, ifmap2_file, output_file):
    """Read saved filter and ifmap files, build and write binary packets with correct bit ordering and convolution windows."""
    # Load filter values (3 rows × 3 cols)
    filter_vals = [int(v) for v in read_txt(filter_file)]
    # Load full 4×4 ifmaps
    ifmap1_all = [int(v) for v in read_txt(ifmap1_file)]
    ifmap2_all = [int(v) for v in read_txt(ifmap2_file)]
    size = int(len(ifmap1_all) ** 0.5)  # should be 4

    # Hop definitions for filter and conv windows
    filter_hops = [('10','0'), ('11','0'), ('00','1'), ('10','1')]
    conv_offsets = [(0,0),(1,0),(0,1),(1,1)]
    conv_hops    = [('00','0'),('01','0'),('00','1'),('01','1')]

    lines = []

    # FILTER PACKETS: 3 rows × 4 destinations
    for row in range(3):
        # 24-bit data: each filter row is 3 bytes
        data_bytes = filter_vals[3*row : 3*row+3]
        data_content = ''.join(f"{b:08b}" for b in data_bytes)
        filter_row = f"{row+1:02b}"  # 01,10,11
        ifmapb = '1'; ts = '0'; direction = '11'
    ...
    # IFMAP PACKETS: 2 maps × 4 convolution windows
    for idx, ifmap_all in enumerate([ifmap1_all, ifmap2_all], start=1):
        ...
        for (x_off, y_off), (x_hop, y_hop) in zip(conv_offsets, filter_hops):
            ...
            # Pack window bits as LSBs of 24-bit data
